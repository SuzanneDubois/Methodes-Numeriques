...
for j in range(8):
  n = pow(2,j+1)
  h = (Xend-Xstart)/n #intervalle
  X = linspace(Xstart,Xend,n+1)
  Uexpl = zeros(n+1); Uexpl[0] = Ustart
  Uimpl = zeros(n+1); Uimpl[0] = Ustart
  for i in range(n):
    Uexpl[i+1] =  Uexpl[i] + h*(sin(X[i]) + Uexpl[i]) #formule euler explicite
    Uimpl[i+1] = (Uimpl[i] + h*sin(X[i+1]))/(1 - h) #formule euler implicite
  Eexpl[j] = abs(Uexpl[-1] - Uend) #erreur à la fin
